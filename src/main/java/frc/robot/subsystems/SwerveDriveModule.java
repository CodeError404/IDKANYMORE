...
public class SwerveDriveModule extends Subsystem {
    ...
    private double driveGearRatio = 6.67; //Was 1
    ...
    public SwerveDriveModule(int moduleNumber, TalonSRX angleMotor, TalonSRX driveMotor, double zeroOffset) {
        ...
        // Set amperage limits
        angleMotor.configContinuousCurrentLimit(30, 0); //(30, 0)
        angleMotor.configPeakCurrentLimit(30, 0); //(30, 0)
        angleMotor.configPeakCurrentDuration(100, 0); //(100, 0)
        angleMotor.enableCurrentLimit(true);

        driveMotor.configContinuousCurrentLimit(30, 0); //(25, 0)
        driveMotor.configPeakCurrentLimit(25, 0); //(25, 0)
        driveMotor.configPeakCurrentDuration(100, 0); //(100, 0)
    ...
    private double encoderTicksToInches(double ticks) {
        if (Robot.PRACTICE_BOT) {
            return ticks / 7; //Was 36.65
        } else {
            return ticks / 6.6; //Was 35.6
        }
    }

    private int inchesToEncoderTicks(double inches) {
        if (Robot.PRACTICE_BOT) {
            return (int) Math.round(inches * 7); //Was 36.65
        } else {
            return (int) Math.round(inches * 6.6); //Was 35.6
        }
    }
